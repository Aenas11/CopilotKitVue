...
| --- | -------------------------- | ------------------------------- | --------------------------------------------- |
| 1   | **Context Sharing Chat**   | AG-UI shared-state conversation | Sidebar state editor + full-width CopilotChat |
...
✅ **Context Isolation** — Each story uses isolated thread state for independent conversations  
...
**Use Case:** Demonstrate AG-UI shared state round-tripping through the runtime and hosted agent  
**UI:** Sidebar state editor plus full-width chat  
**Interaction Pattern:** Update `userName` / `currentPage`, then ask state-aware questions such as "what page am I on?"
...
- ✅ `useAgent` — direct access to shared state, messages, and run status
- ✅ `useFrontendTool` — client-side tool registration
...
- ✅ Isolated threadId per live story
- ✅ Dynamic thread switching (story 3)

### Validated Live Behavior

- ✅ `Context Sharing Chat` now uses AG-UI shared state instead of advisory frontend context
- ✅ Verified in Storybook: asking "what page am i on?" returned "You're on the Dashboard page. Your username is Alex."
- ✅ Backend state snapshots are reflected in the sidebar inspector during the run
...
- Isolated thread ids per story, with the context-sharing story minting a fresh thread per mount for clean live validation
...
1. **Composable Integration Stories** — Expand the existing live stories with more AG-UI state mutation scenarios and richer multi-step state flows
...
- ✅ `examples/agent/SharedStateAgent.cs` — grounded second-pass answers on resolved AG-UI state snapshots
- ✅ `packages/vue/src/stories/LiveAgentAdvancedScenarios.stories.ts` — context-sharing story now uses AG-UI shared state with fresh per-mount thread ids
- ✅ `packages/vue/src/stories/ADVANCED_SCENARIOS.md` — updated to describe AG-UI shared-state behavior
